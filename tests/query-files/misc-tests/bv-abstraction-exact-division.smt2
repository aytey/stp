...
; abstracted at all.
;
; RUN: %solver --incremental=off -d -s --bv-term-abstraction=1 --bv-eq-abstraction-width=1 --bv-term-abstraction-rounds=1 %s 2>&1 | %OutputCheck --check-prefix=ABSTRACTED %s
