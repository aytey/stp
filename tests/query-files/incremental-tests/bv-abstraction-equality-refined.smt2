...
;
; RUN: %solver --incremental=on -d --bv-eq-abstraction=1 --bv-eq-abstraction-width=1 %s 2>&1 | %OutputCheck --check-prefix=ABSTRACTED %s
; RUN: %solver --incremental=off -d --bv-eq-abstraction=1 --bv-eq-abstraction-width=1 %s 2>&1 | %OutputCheck --check-prefix=PLAIN %s
;
