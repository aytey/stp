...
;
; RUN: %solver --incremental=on -d --array-equality --bv-eq-abstraction=1 --bv-term-abstraction=1 --bv-eq-abstraction-width=1 %s 2>&1 | %OutputCheck --check-prefix=ABSTRACTED %s
; RUN: %solver --incremental=on -d --array-equality %s 2>&1 | %OutputCheck --check-prefix=PLAIN %s
; RUN: %solver --incremental=off --array-equality --bv-eq-abstraction=1 --bv-term-abstraction=1 --bv-eq-abstraction-width=1 %s 2>&1 | %OutputCheck --check-prefix=ABSTRACTED %s
;
