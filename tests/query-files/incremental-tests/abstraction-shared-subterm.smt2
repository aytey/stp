...
; free to disagree. The registry refinement reads a record's result through
; holds one vector per node, so the second registration hides the first:
; both records are then defined over the second set and the first is left
; unconstrained, which is an assignment the search may answer from.
;
