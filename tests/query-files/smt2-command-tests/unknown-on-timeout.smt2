...
; the operands, with the ordering rewrite off so the query survives to the
; solver.
;
; RUN: %solver --distinct-ordering=0 -g 2 %s 2>&1 | %OutputCheck --check-prefix=CONFL %s
; RUN: %solver --distinct-ordering=0 -k 0 %s 2>&1 | %OutputCheck --check-prefix=CLOCK %s
; RUN: %solver --distinct-ordering=0 -k 0 -g 2 %s 2>&1 | %OutputCheck --check-prefix=CLOCK %s
;
; CONFL: ^unknown$
