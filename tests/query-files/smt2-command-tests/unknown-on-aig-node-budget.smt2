...
;
; The second leg is the control. At a budget of 0 there is no limit and the
; same query is answered, which is what says the first leg exhausted a budget
; rather than merely found the query hard.
;
; RUN: %solver --incremental=off --aig-node-budget=100 %s 2>&1 | %OutputCheck --check-prefix=CAPPED %s
; RUN: %solver --incremental=off --aig-node-budget=0 %s 2>&1 | %OutputCheck --check-prefix=UNCAPPED %s
;
; CAPPED: ^unknown$
; CAPPED: :reason-unknown \(incomplete "the AIG node budget set by --aig-node-budget \(100\) ran out at [0-9]+ AND gates; raise it, or set it to 0 for no limit"\)
