...
TEST(IncrementalScopeState, ReconcileVersionsChangedSuffixesAndTracksStability)
{
  ...
  const uint64_t oldTopId = scopes.frame(1).id;
  ...
  EXPECT_NE(oldTopId, scopes.frame(1).id);
...
TEST(IncrementalScopeState, NewScopeIdentityCanStillReuseContentMemo)
{
  ...
  // frame identity is new, so the processed engine prefix must roll back;
...
TEST(IncrementalScopeState, PromotionIsOwnedByThePreparedFrame)
{
  STPMgr mgr;
  IncrementalScopeState scopes;
