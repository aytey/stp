...
TEST(bv_abstraction_array_candidate_is_bound, array_disequality_inside_an_assumption_scope)
{
  ...
  Expr maxSigned = vc_bvConstExprFromStr(vc, std::string("0" + std::string(WIDE - 1, '1')).c_str());
  Expr minSigned = vc_bvConstExprFromStr(vc, std::string("1" + std::string(WIDE - 1, '0')).c_str());
  Expr zero = vc_bvConstExprFromStr(vc, std::string(WIDE, '0').c_str());
  ...
  UFDeclHandle f = vc_declareUninterpretedFunction(vc, "f", unaryDomain, 1, wide);
  ASSERT_NE(0u, f);

  Type predicateDomain[4] = {wide, vc_boolType(vc), wide, vc_boolType(vc)};
  UFDeclHandle p = vc_declareUninterpretedFunction(vc, "p", predicateDomain, 4,
                                                   vc_boolType(vc));
  ASSERT_NE(0u, p);
  ...
  Expr array = vc_varExpr(vc, "a", vc_arrayType(vc, vc_bvType(vc, INDEX), wide));
  ...
  Expr storeAtConst = vc_writeExpr(
      vc, array, vc_bvConstExprFromStr(vc, "01111011000001100101000100110101"),
      quotient);
  Expr storesDiffer =
